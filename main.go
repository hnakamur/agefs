...
func main() {
	...
	mntDir, _ := os.CreateTemp("", "")
	defer func() {
		if err := os.Remove(mntDir.Name()); err != nil {
			log.Printf("failed to delete temporary mount directory %s: %s", mntDir.Name(), err)
		}
	}()

	root := &inMemoryFS{}
	server, err := fs.Mount(mntDir.Name(), root, &fs.Options{
	...
	log.Printf("Mounted on %s", mntDir.Name())
	...
	log.Print("Unmount and exit by pressing Ctrl-C")
